...
int test_une_fonction(char *nom_fonc, int exist_p, int (*test_fonc)(void)) {
    ...
    printf_test("%s %*s ", nom_fonc,  36 - (int) strlen(nom_fonc), "");
    if (exist_p) {
	if (test_fonc()) {
...
#pragma weak TEST_ERREUR_FATALE
extern int TEST_ERREUR_FATALE;

void tester_erreur_fatale(void) {
    TEST_ERREUR_FATALE = 1;
}

int verifier_erreur_fatale(void) {
    int test = (TEST_ERREUR_FATALE == 0);
...
#define RUN_TEST_FATALE(CODE) run_test(                                 \
    (tester_erreur_fatale(),CODE,verifier_erreur_fatale()),             \
    #CODE,                                                              \
    __FILE__,                                                           \
    __LINE__)
...
#pragma weak pointeur_vers_indice
int pointeur_vers_indice(void *);

#pragma weak ramasse_miettes_lire_marque
int ramasse_miettes_lire_marque(void *);

#pragma weak ramasse_miettes_poser_marque
int ramasse_miettes_poser_marque(void *);

#pragma weak bloc_libre
int bloc_libre(int);

#pragma weak ramasse_miettes_liberer
int ramasse_miettes_liberer(void);

#pragma weak ramasse_miettes_parcourir_et_marquer
void ramasse_miettes_parcourir_et_marquer(sexpr);

#pragma weak valisp_ramasse_miettes
void valisp_ramasse_miettes(sexpr);

#pragma weak erreur_fatale
...
char *IMAGE_TEST_RM_SIMPLE ="rm-simple.img";
char *IMAGE_TEST_RM_SIMPLE_M ="rm-simple-m.img";
char *IMAGE_TEST_RM_SIMPLE_L ="rm-simple-l.img";
char *IMAGE_TEST_RM   ="rm.img";
char *IMAGE_TEST_RM_M ="rm-m.img";
char *IMAGE_TEST_RM_L ="rm-l.img";
...
int run_test_pointeur_vers_indice(void) {
    int i = 0;
    int b = 1;
    int n = 1<<15;
    for (i=1 ; i<n ; i++) {
        b = b && RUN_TEST(pointeur_vers_indice(&MEMOIRE_DYNAMIQUE[i]) == i-1);
    }
    b = b && RUN_TEST_FATALE(pointeur_vers_indice(NULL));
    b = b && RUN_TEST_FATALE(pointeur_vers_indice((void *) 1));
    b = b && RUN_TEST_FATALE(pointeur_vers_indice(MEMOIRE_DYNAMIQUE - 1));
    b = b && RUN_TEST_FATALE(pointeur_vers_indice(MEMOIRE_DYNAMIQUE + (1<<15)));
    return b;
}

int run_test_ramasse_miettes_lire_marque(void) {
    struct scenario_quelconque scenar = test_1_3_b();
    int b = 1;
    char *binaire;
    int i;
    ...
    for (i=0; i<scenar.n; i++){
        ...
        binaire = scenar.chaines[i];
        voulu   = prof_binaire_vers_rm(binaire);
	obtenue = ramasse_miettes_lire_marque(&MEMOIRE_DYNAMIQUE[indice+1]) ;
...
int run_test_ramasse_miettes_poser_marque(void) {
    struct scenario_quelconque scenar = test_1_3_b();
    int b = 1;
    void *p;
    int i;
    ...
    for (i=0; i<scenar.n; i++){
        p = &MEMOIRE_DYNAMIQUE[indice+1];
        indice  = scenar.indices[i];
        obtenue = ramasse_miettes_lire_marque(p) ;
        if (obtenue==1) {
            b = b && RUN_TEST_FATALE(ramasse_miettes_poser_marque(p));
        } else {
            ramasse_miettes_poser_marque(p);
            b = b && RUN_TEST(ramasse_miettes_lire_marque(p) == 1);
        }
...
int un_test_bloc_libre(char *image) {
   int b = 1, i;
    charger_image(image);
    for (i=0 ; (MEMOIRE_DYNAMIQUE[i]  & 0x7FFF) != i ; i = MEMOIRE_DYNAMIQUE[i] & 0x7FFF) {
        b = b && RUN_TEST(rm_bloc(i) != bloc_libre(i));
        if (!b) {
            printf("Erreur à l’indice i=%d\n",i);
            printf("Pour charger l’image %s@load %s%s\n", couleur_jaune, image, couleur_defaut);
            break;
        }
    }
    b = b && RUN_TEST(!bloc_libre((1<<15) - 1));
    return b;
}

int run_test_bloc_libre(void) {
    int b = 1;
    b = b && un_test_bloc_libre(IMAGE_TEST_RM_SIMPLE_M);
    b = b && un_test_bloc_libre(IMAGE_TEST_RM_M);
    return b;
}


void expliquer_erreur_rm(char *origine, char *destnation, char *directive, char *nom_fonction) {
       printf("%sPour comprendre l’erreur, faite %s\n", couleur_gras, couleur_defaut);
        printf("    %s@load %s%s pour charger une image\n",
               couleur_jaune, origine, couleur_defaut);
        printf("    %s@mem%s pour voir la mémoire\n",
               couleur_jaune,  couleur_defaut);
        printf("    %s%s%s   pour appeller la fonction %s\n",
               couleur_jaune,  directive, couleur_defaut, nom_fonction);

        printf("\n%sVous devez obtenir la même mémoire qu’avec%s \n", couleur_gras, couleur_defaut);
        printf("    %s@load %s%s\n",
               couleur_jaune,  destnation, couleur_defaut);
        printf("    %s@mem%s \n\n",
               couleur_jaune,  couleur_defaut);
...
int un_test_ramasse_miettes_liberer(char *origine, char *destination)  {
    int b = 1;
    charger_image(origine);
    ramasse_miettes_liberer();
    b = comparer_image_et_memoire(destination);
    if (!b) {
        printf("%s[ÉCHEC]%s\n\n", couleur_rouge, couleur_defaut);
        expliquer_erreur_rm(origine,
                            destination,
                            "@l",
                            "ramasse_miettes_liberer");
    }
...
int run_test_ramasse_miettes_liberer(void) {
    int b = 1;
    b = b && un_test_ramasse_miettes_liberer(IMAGE_TEST_RM_SIMPLE_M, IMAGE_TEST_RM_SIMPLE_L);
    b = b && un_test_ramasse_miettes_liberer(IMAGE_TEST_RM_M, IMAGE_TEST_RM_L);
    return b;
}

int un_test_rm_parcourir_et_marquer(char *origine, char *destination) {
    int b = 1;
    charger_image(origine);
    ...
    if (!b) {
        ...
        expliquer_erreur_rm(origine,
                            destination,
                            "@m",
                            "ramasse_miettes_parcourir_et_marquer");
    }
    return b;
}
...
int run_test_ramasse_miettes_parcourir_et_marquer(void) {
    int b = 1;
    b = b && un_test_rm_parcourir_et_marquer(IMAGE_TEST_RM_SIMPLE, IMAGE_TEST_RM_SIMPLE_M);
    b = b && un_test_rm_parcourir_et_marquer(IMAGE_TEST_RM, IMAGE_TEST_RM_M);
    return b;
}


int run_test_valisp_ramasse_miettes(void) {
    int b = 1;
    charger_image(IMAGE_TEST_RM_SIMPLE);
    valisp_ramasse_miettes(ENV);
    b = comparer_image_et_memoire(IMAGE_TEST_RM_SIMPLE_L);
    if (!b) printf("%s[ÉCHEC]%s\n\n", couleur_rouge, couleur_defaut);
    return b;
}
...
int test_tp3(void) {
    ...
    b = b && TEST_FONCTION(pointeur_vers_indice);                 nombre_test_valides += b;
    b = b && TEST_FONCTION(ramasse_miettes_lire_marque);          nombre_test_valides += b;
    b = b && TEST_FONCTION(ramasse_miettes_poser_marque);         nombre_test_valides += b;
    b = b && TEST_FONCTION(bloc_libre);                           nombre_test_valides += b;
    b = b && TEST_FONCTION(ramasse_miettes_liberer);              nombre_test_valides += b;
    b = b && TEST_FONCTION(ramasse_miettes_parcourir_et_marquer); nombre_test_valides += b;
    b = b && TEST_FONCTION(valisp_ramasse_miettes);               nombre_test_valides += b;
    return nombre_test_valides;
}
